...
public interface AnimalsRepository extends JpaRepository<Animals, Integer> {
        ...
        // 전체검색만!!
        @Query(value = "SELECT * FROM Animals am WHERE am.orgNm = :addrSido +:addrSigungu GROUP BY age, careAddr, careNm, careTel, chargeNm, colorCd, desertionNo, filename, happenDt, happenPlace, kindCd, neuterYn, noticeComment, noticeEdt, noticeNo, noticeSdt, officetel, orgNm, popfile, processState, sexCd, specialMark, weight HAVING am.orgNm = :addrSido +:addrSigungu ORDER BY am.orgNm = :addrSido +:addrSigungu", nativeQuery = true)
