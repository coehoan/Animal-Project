...
async function toAll(selectedKind, selectedKindOf, selectedSido, selectedSigungu, firstDate, lastDate) { // 지역 검색
    ...
    let response = await fetch(`/search/animals/all?&kind=${selectedKind}&kindOf=${selectedKindOf}&sido=${selectedSido}&sigungu=${selectedSigungu}&firstdate=${firstDate}&lastdate=${lastDate}`);
    console.log(selectedSido, selectedSigungu, selectedKind, selectedKindOf, firstDate, lastDate);
    console.log(response);
...
async function toDay(firstDate, lastDate) { // 날짜 검색
    ...
    let response = await fetch(`/search/animals/day?firstdate=${firstDate}&lastdate=${lastDate}`)
